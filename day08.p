...
/*------------------------------------------------------------------------
    ...
    Created     : Mon Dec 08 13:55:02 CET 2025
    Notes       :
  ----------------------------------------------------------------------*/
...
DEFINE TEMP-TABLE ttBox
   ...
   FIELD iY    AS INTEGER 
   FIELD iZ    AS INTEGER 
INDEX indBox IS UNIQUE IDBox.

DEFINE TEMP-TABLE ttDelta
   FIELD IDBox_From AS INTEGER 
   FIELD IDBox_To   AS INTEGER 
   FIELD deDistance AS DECIMAL 
INDEX indDelta    IS UNIQUE IDBox_From IDBox_To
INDEX indDistance IS PRIMARY deDistance.  
...
FUNCTION getDistance3 RETURNS DECIMAL 
   (INPUT ipiP AS INTEGER EXTENT,
    INPUT ipiQ AS INTEGER EXTENT) FORWARD.
...
DO:
   ...
   MESSAGE 
      ...
      VIEW-AS ALERT-BOX TITLE " 2025 - Day 08 - Part One".
...
DO:
   ...
   MESSAGE 
      ...
      iSolution) SKIP (1)
      SUBSTITUTE ("Found solution in &1 msecs.", ETIME)
      VIEW-AS ALERT-BOX TITLE " 2025 - Day 08 - Part Two".
END. /* Process Part One */
...
FUNCTION getDistance3 RETURNS DECIMAL 
(INPUT ipiP AS INTEGER EXTENT,
 INPUT ipiQ AS INTEGER EXTENT):
/*------------------------------------------------------------------------------
 Purpose: Returns the Euclidean distance between two (P and Q) points in 3D 
 Notes:   See: https://en.wikipedia.org/wiki/Euclidean_distance
------------------------------------------------------------------------------*/   
DEFINE VARIABLE deDistance AS DECIMAL NO-UNDO.

DEFINE VARIABLE iMaxDim   AS INTEGER NO-UNDO.
DEFINE VARIABLE iDim      AS INTEGER NO-UNDO.
DEFINE VARIABLE iDeltaSqr AS INT64   NO-UNDO EXTENT.
DEFINE VARIABLE iTotalSqr AS INT64   NO-UNDO.

   iMaxDim = EXTENT (ipiP).
      
   EXTENT(iDeltaSqr) = iMaxDim.
   DO iDim = 1 TO iMaxDim:
      iDeltaSqr[iDim] = EXP (ipiP[iDim] - ipiQ[iDim], 2).
      iTotalSqr += iDeltaSqr[iDim].
   END.
   
   deDistance = SQRT (iTotalSqr).
